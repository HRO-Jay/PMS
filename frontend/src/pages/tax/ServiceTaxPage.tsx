...
const ServiceTaxPage: React.FC = () => {
  ...
  return (
    <Card size="small" title="劳务个税计算（劳务报酬，计税方式为劳务计税的人员）">
      ...
      <Table columns={columns} dataSource={records} loading={loading} scroll={{ x: 800, y: 'calc(100vh - 280px)' }} size="small" pagination={{ pageSize: 50, showSizeChanger: true, showTotal: t => `共 ${t} 条` }} />
    </Card>
  );
};
