...
const TaxMonthlyCalcPage: React.FC = () => {
  ...
  return (
    <Card size="small" title="个税月度计算（累计预扣法，正常计税人员）">
      ...
      <Table columns={columns} dataSource={records} loading={loading} scroll={{ x: 2000, y: 'calc(100vh - 280px)' }} size="small" pagination={{ pageSize: 50, showSizeChanger: true, showTotal: t => `共 ${t} 条` }} />
    </Card>
  );
};
